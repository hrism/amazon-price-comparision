...
-- コメント追加
COMMENT ON COLUMN toilet_paper_products.total_score IS '総合評価スコア（0-5点、レビュー70%・価格30%の重み付け）';
COMMENT ON COLUMN dishwashing_liquid_products.total_score IS '総合評価スコア（0-5点、レビュー70%・価格30%の重み付け）';
COMMENT ON COLUMN mineral_water_products.total_score IS '総合評価スコア（0-5点、レビュー70%・価格30%の重み付け）';
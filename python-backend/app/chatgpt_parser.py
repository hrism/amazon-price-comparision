...
- roll_count: 実際に換算すべきロール数（数値のみ、単位なし）
- length_m: 1ロールあたりの長さ（メートル単位、数値のみ）
- is_double: ダブルかシングルか（true/false/null）

重要な注意事項：
1. 「2倍巻」「3倍巻」「長持ち」商品の場合：
   - 「12ロールで24ロール分」→ roll_count: 24（通常ロール換算数を使用）
   - 「4ロールで12ロール分」→ roll_count: 12（通常ロール換算数を使用）
   - 物理的なロール数ではなく、換算後のロール数を使用してください
...
出力: {{"roll_count": 24, "length_m": 50, "is_double": true}}
説明: 2倍巻きなので、物理的12ロールを通常24ロール分として換算

例2：
入力: "スコッティ フラワーパック 3倍長持ち 75m×12ロール (12ロールで36ロール分)"
出力: {{"roll_count": 36, "length_m": 75, "is_double": true}}
説明: 3倍長持ちなので、物理的12ロールを通常36ロール分として換算
